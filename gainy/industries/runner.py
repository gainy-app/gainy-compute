...
class IndustryAssignmentRunner:
    ...
    def run_train(self):
        ...
        tickers_with_industries = tickers.merge(manual_industries, how="inner", on=["symbol"])

        self._set_mlflow_experiment()
        with mlflow.start_run() as run:
            x_score = mean(self._cross_validation(tickers_with_industries))
            logging.info(f"Model cross validation score (average MAP) is {x_score}")
            ...
            logging.info(f"Model train score (MAP on training data) is {train_score}")
            mlflow.log_metric("Train MAP", train_score)

            mlflow.pyfunc.log_model(artifact_path=self.model.name(), python_model=self.model)

            if x_score < self.MIN_X_SCORE:
                logging.error(f"The cross-validation score is too low, expected={self.MIN_X_SCORE}, actual={x_score}")
                raise Exception("The cross-validation score is too low, review the data and run it again.")
            else:
                self._register_model(run)

    def _cross_validation(self, tickers_with_industries, n_splits: int = 3):
        industry_counts = tickers_with_industries[["industry_id", "symbol"]].groupby("industry_id").count()
        ...
        tickers_with_industries = tickers_with_industries.merge(selected_industries, on=["industry_id"], how="inner")
    ...
    def _register_model(self, run):
        logging.info(f"Register model `{self._registered_name}` with run_id `{run.info.run_id}`")
        model_version = mlflow.register_model(f"runs:/{run.info.run_id}", self._registered_name)
    ...
    def run_predict(self):
        ...
        tickers = tickers[tickers["description"] == tickers["description"]]  # Remove Nones, NaNs, etc
        ...
        for start in range(0, len(tickers), batch_size):
            predictions_list += self.model.predict(ticker_descriptions.iloc[start:start + batch_size], n=2, include_distances=False)
...
#         predictions = pd.DataFrame(data=predictions_list, columns=["industry_id_1", "industry_id_2"])
    ...
    def _load_model(self):
        client = MlflowClient()
        latest_version = client.get_latest_versions(self._registered_name, [self._model_version_stage])[0]
        artifact_uri = client.get_model_version_download_uri(latest_version.name, latest_version.version)
