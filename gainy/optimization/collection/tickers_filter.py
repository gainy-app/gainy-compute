...
class CollectionTickerFilter:
    ...
    def filter(self, tickers, min_market_cap=100, min_volume=2, min_price=1) -> list:
        """
        ...
        df.loc[df.marketcap < min_market_cap, 'Flag'] = str(df.loc[df.marketcap < min_market_cap, 'Flag'].values) + '- MC'
        df.loc[df.adjusted_close < min_price, 'Flag'] = str(df.loc[df.adjusted_close < min_price, 'Flag'].values) + '-Price'
        df.loc[df.vol_doll < min_volume, 'Flag'] = str(df.loc[df.vol_doll < min_volume, 'Flag'].values) + '-Volume'
