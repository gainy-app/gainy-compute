...
class OptimizeCollectionsJob:
    ...
    def run(self, collection_id: int, date: datetime.date, output_filename: str):
        if collection_id:
        ...
        for collection_id in collection_ids:
            ...
            df.to_csv(output_filename, index=False, mode='a', header=(not os.path.exists(output_filename)))
    ...
    def _optimize_collection(self, collection_id: int, date: datetime.date):
        ...
        optimizer = PortfolioRiskBudgetCollectionOptimizer(self.repository, tickers, date, benchmark='SPY', lookback=9, **self.params)
        opt_res = optimizer.optimize()
        logger.info("Optimization result %s", opt_res)

        return opt_res

    def _opt_res_to_df(self, collection_id: int, opt_res: dict, date: datetime.date) -> pd.DataFrame:
...
def cli(args=None):
    ...
    parser.add_argument('--id', dest='collection_id', type=int, help='Collection id')
    ...
    try:
        with ContextContainer() as context_container:
            job = OptimizeCollectionsJob(context_container.collection_optimizer_repository)
            job.run(collection_id=collection_id, date=date, output_filename=output_filename)
